  ...
  const services = [
    {
      ...
      cases: [
        {
          id: "case-1-notified-body-finding-remediation",
          title: {
            es: "Remediación de Finding de Notified Body en Supplier Management",
            en: "Notified Body Finding Remediation in Supplier Management"
          },
          situation: {
            es: "Startup de dispositivos médicos había sido recientemente adquirida por empresa más grande, resultando en cambios significativos en estructura de distribución. Durante auditoría de surveillance, Notified Body identificó finding mayor: documentación de supplier management contenía referencias a distribuidores que ya no se utilizaban debido a integración post-adquisición—los nuevos distribuidores eran parte de la empresa compradora pero no estaban documentados apropiadamente en el sistema de calidad. Análisis adicional reveló problemas sistémicos más profundos: procedimiento de supplier management carecía de criterios claros para selección y evaluación de proveedores, formas y templates para supplier records eran ambiguos sin guidance clara sobre qué evidencia documentar, no existía proceso formal de performance evaluation de proveedores—ni descrito en procedimiento ni evidenciado en records, supplier qualification packages inconsistentes entre proveedores con gaps en documentación crítica. Sistema funcionaba operativamente pero documentation framework era inadecuado para demostrar cumplimiento regulatorio robusto.",
            en: "Medical device startup had been recently acquired by larger company, resulting in significant changes in distribution structure. During surveillance audit, Notified Body identified major finding: supplier management documentation contained references to distributors no longer used due to post-acquisition integration—new distributors were part of acquiring company but not properly documented in quality system. Additional analysis revealed deeper systemic problems: supplier management procedure lacked clear criteria for supplier selection and evaluation, forms and templates for supplier records were ambiguous without clear guidance on what evidence to document, no formal supplier performance evaluation process existed—neither described in procedure nor evidenced in records, inconsistent supplier qualification packages between suppliers with gaps in critical documentation. System functioned operationally but documentation framework was inadequate to demonstrate robust regulatory compliance."
          },
          solution: {
            es: "Implementamos remediación comprehensiva de supplier management system: (1) ACTUALIZACIÓN DE DISTRIBUIDORES: Documentamos cambio de distribuidores post-adquisición mediante formal change control, creamos supplier qualification packages para nuevos distribuidores (parte de empresa compradora) con appropriate documentation, actualizamos procedimientos y registros eliminando referencias obsoletas a distribuidores anteriores, establecimos clear distribution agreements y responsibilities, (2) REDISEÑO DE PROCEDIMIENTO: Reescribimos procedimiento de supplier management con claridad significativamente mejorada: definimos criterios objetivos para supplier selection basados en risk-based approach (criticality of product/service, device risk class, supplier capabilities), establecimos step-by-step evaluation process con clear deliverables en cada fase, incorporamos requisitos específicos para diferentes tipos de suppliers (manufacturers, service providers, distributors, raw material suppliers), agregamos section dedicada a supplier performance evaluation con frequency, metrics, y process definidos, (3) DESARROLLO DE FORMS Y TEMPLATES: Creamos comprehensive supplier record templates con clear instructions: Supplier Evaluation Form con scoring criteria y weighted factors, Supplier Audit Checklist specific para on-site evaluations, Quality Agreement template con standard clauses y customizable sections, Supplier Performance Scorecard con defined KPIs (quality metrics, delivery performance, responsiveness, compliance), Annual Supplier Review template para documented re-evaluation, (4) IMPLEMENTACIÓN DE PERFORMANCE EVALUATION: Establecimos formal supplier performance monitoring program: defined KPIs relevantes para medical device requirements (defect rates, on-time delivery, CAPA responsiveness, audit findings, certification status), implemented quarterly scorecard reviews para critical suppliers, established annual review process para all active suppliers con documented outcomes, created performance-based supplier classification system informando frequency de monitoring, (5) RETROACTIVE DOCUMENTATION: Completamos supplier performance evaluations retrospectively para existing suppliers usando available data (receiving inspection records, CAPA history, delivery performance), documented justification para suppliers sin complete historical data, established baseline para ongoing monitoring, (6) TRAINING: Entrenamos quality team y procurement personnel en updated procedures, forms, y expectations, conducted practical exercises usando real supplier scenarios, established clear roles y responsibilities para supplier management activities.",
            en: "We implemented comprehensive supplier management system remediation: (1) DISTRIBUTOR UPDATE: Documented post-acquisition distributor change through formal change control, created supplier qualification packages for new distributors (part of acquiring company) with appropriate documentation, updated procedures and records eliminating obsolete references to previous distributors, established clear distribution agreements and responsibilities, (2) PROCEDURE REDESIGN: Rewrote supplier management procedure with significantly improved clarity: defined objective criteria for supplier selection based on risk-based approach (criticality of product/service, device risk class, supplier capabilities), established step-by-step evaluation process with clear deliverables at each phase, incorporated specific requirements for different supplier types (manufacturers, service providers, distributors, raw material suppliers), added dedicated section for supplier performance evaluation with defined frequency, metrics, and process, (3) FORMS AND TEMPLATES DEVELOPMENT: Created comprehensive supplier record templates with clear instructions: Supplier Evaluation Form with scoring criteria and weighted factors, Supplier Audit Checklist specific for on-site evaluations, Quality Agreement template with standard clauses and customizable sections, Supplier Performance Scorecard with defined KPIs (quality metrics, delivery performance, responsiveness, compliance), Annual Supplier Review template for documented re-evaluation, (4) PERFORMANCE EVALUATION IMPLEMENTATION: Established formal supplier performance monitoring program: defined KPIs relevant to medical device requirements (defect rates, on-time delivery, CAPA responsiveness, audit findings, certification status), implemented quarterly scorecard reviews for critical suppliers, established annual review process for all active suppliers with documented outcomes, created performance-based supplier classification system informing monitoring frequency, (5) RETROACTIVE DOCUMENTATION: Completed supplier performance evaluations retrospectively for existing suppliers using available data (receiving inspection records, CAPA history, delivery performance), documented justification for suppliers without complete historical data, established baseline for ongoing monitoring, (6) TRAINING: Trained quality team and procurement personnel on updated procedures, forms, and expectations, conducted practical exercises using real supplier scenarios, established clear roles and responsibilities for supplier management activities."
          },
          results: {
            es: "Notified Body finding cerrado exitosamente en primera revisión: auditor confirmó que remediaciones abordaban comprehensivamente tanto specific issue (distribuidores obsoletos) como root causes sistémicas (procedimientos ambiguos, falta de performance evaluation). Documentación de supplier management transformada: procedimiento claro con step-by-step guidance eliminando ambiguity, forms y templates standardizados asegurando consistency y completeness, supplier qualification packages ahora uniformes con evidencia apropiada. Performance evaluation program implementado y functioning: 100% de suppliers críticos tienen quarterly scorecards, 100% de active suppliers tienen annual reviews documentadas, system genera actionable data informando supplier management decisions. Efficiency mejorada: tiempo para completar supplier qualification redujo 35% debido a clear templates y guidance, quality team confidence aumentó—saben exactamente qué documentar y cómo evaluar. Auditorías subsecuentes: siguiente Notified Body surveillance audit resultó en zero findings en supplier management área—auditor comentó improvement significativo. Sistema escalable: framework implementado soportó crecimiento de supplier base de 45 a 75 suppliers durante siguientes 18 meses sin procedural changes requeridos. Cultural shift: organización pasó de reactive compliance a proactive supplier quality management con data-driven decisions.",
            en: "Notified Body finding closed successfully on first review: auditor confirmed remediations comprehensively addressed both specific issue (obsolete distributors) and systemic root causes (ambiguous procedures, lack of performance evaluation). Supplier management documentation transformed: clear procedure with step-by-step guidance eliminating ambiguity, standardized forms and templates ensuring consistency and completeness, supplier qualification packages now uniform with appropriate evidence. Performance evaluation program implemented and functioning: 100% of critical suppliers have quarterly scorecards, 100% of active suppliers have documented annual reviews, system generates actionable data informing supplier management decisions. Improved efficiency: time to complete supplier qualification reduced 35% due to clear templates and guidance, quality team confidence increased—they know exactly what to document and how to evaluate. Subsequent audits: next Notified Body surveillance audit resulted in zero findings in supplier management area—auditor commented on significant improvement. Scalable system: implemented framework supported supplier base growth from 45 to 75 suppliers over next 18 months without required procedural changes. Cultural shift: organization moved from reactive compliance to proactive supplier quality management with data-driven decisions."
          }
        },
        {
      ...
      cases: [
        {
          ...
          results: {
            es: "Historial regulatorio impecable: cero hallazgos en controles de compras o gestión de proveedores en todas las inspecciones internacionales (MDD/MDR, Corea, Japón) durante 2 años—demostración objetiva de sistema robusto. Programa de elevación completado exitosamente preparando empresa para inspección pre-aprobación de PMA con FDA con nivel alto de confianza. Sistema mejorado mantiene balance óptimo: cumple con expectativas más exigentes de FDA sin agregar burocracia innecesaria que comprometería agilidad de startup. Inversión estratégica en elevación de QMS posicionó empresa para: abordar inspección de FDA desde posición de fortaleza vs cumplimiento reactivo, aprovechar historial internacional durante discusiones regulatorias, capacidad de demostrar cumplimiento sostenido y cultura de mejora continua. Documentación comprehensiva y controles sistémicos desarrollados durante programa sirven como fundación escalable: sistema puede crecer con empresa desde startup hasta etapa comercial sin revisión mayor. Equipo transformado: científicos sin experiencia regulatoria desarrollaron entendimiento profundo de expectativas de FDA, capaces de navegar requisitos de cumplimiento con confianza, preparados para comunicarse efectivamente con inspectores.",
